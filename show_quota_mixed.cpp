...
void print_usage(const char* program_name) {
    ...
    std::cerr << "  --gui, -g            Launch GUI mode" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help" || arg == "-h") {
            print_usage(argv[0]);
            return 0;
        } else if (arg == "--gui" || arg == "-g" || arg == "--gui-resizable" || arg == "--gui-tiny") {
            // All GUI options just enable GUI mode (--gui-resizable and --gui-tiny are legacy)
            gui_mode = true;
        } else if (arg == "-1") {
            refresh_interval = 0;
    ...
    if (gui_mode) {
#ifdef GUI_MODE_ENABLED
        result = run_gui_mode(api_key, refresh_interval, log_file, logging_enabled, &argc, &argv);
        curl_global_cleanup();
        return result;
#else
